...
void AudioTrack::releaseBuffer(Buffer* audioBuffer)
{
    ...
    {
//	ALOGD("AudioTrack::releaseBuffer begin....");	
	#ifdef AUDIO_GAIN

		int out = 0;
		int index = 0;
		
		int dataBytes = audioBuffer->size / 2;
		short *audio = (short *)audioBuffer->i16;

		float gain  = 1.0f;
		char gainBuf[256] = {0};

		property_get("orion.audio.gain",gainBuf,"1");		
		sscanf(gainBuf, "%f", &gain);

//		ALOGD("gain = %f",gain);
		
		for(index = 0;index < dataBytes;index++) {
			 out = (int)(gain * audio[index]);
			 audio[index] = max(-32768, min(32767, out));
		}

		buffer.mRaw = (void *)audio;
/*		
		static FILE * spkFile = fopen("/data/local/test/spkFile.pcm","wb");		
		
		fwrite((char *)audioBuffer->i16,audioBuffer->size,1,spkFile);
*/		
	#endif

//	ALOGD("AudioTrack::releaseBuffer end....");
	}
